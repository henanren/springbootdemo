...
//	@PostConstruct
	...
	public String test() {
		...
		Transaction t = Cat.getProducer().newTransaction("Transaction-test1", "your transaction name");
		try {
			...
			Cat.getProducer().logEvent("Event-test1", "your event name", Event.SUCCESS, "keyValuePairs");
			t.setStatus(Transaction.SUCCESS);
		} catch (Exception e) {
			Cat.getProducer().logError(e);
			// 用log4j记录系统异常，以便在Logview中看到此信息
